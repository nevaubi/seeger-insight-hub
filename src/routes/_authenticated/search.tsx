...
// Planner step — streams the model's reasoning tokens, then collapses to facet chips.
function PlanningStep({
  ...
  const shown = useSmoothText(planReasoning, streaming, 220);
  const words = useRevealedWords(shown);
  const hasFacets = !!plan && plan.facets.length > 0;
  return (
    <div className="flex items-start gap-3 motion-fade-rise">
      <div className="flex shrink-0 justify-center pt-[3px]" style={{ width: NODE_COL }}>
        <div className={`relative z-10 grid h-[14px] w-[14px] place-items-center rounded-full ${streaming ? 'bg-gold motion-pulse' : 'bg-gold'}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5 leading-[16px]">
          ...
          {planModel && (
            <span className="text-[10px] text-muted-foreground/60">· {planModel}</span>
          )}
          {hasFacets && (
        ...
        {shown && (
          <p className="mt-1 font-serif text-[13px] leading-[1.55] text-foreground/80">
            {words.map((w) =>
              w.fresh ? (
      ...
      <div className="overflow-hidden" ref={reasoningScrollRef}>
        <div className="relative py-2">
          ...
          <div className="space-y-6">
            {(planStreaming || planReasoning || (plan && plan.facets.length > 0)) && (
              <PlanningStep
                plan={plan}
                planReasoning={planReasoning}
                planModel={planModel}
